...
async def FileEmptyCheck(FileListObj, ctx):
    ...
    try:
        ...
        logger.error(
            f"Uncaught Exception in Module : `FileEmptyCheck`, ErrorType : `{type(Exc)}` : {Exc}")
    else:
        return(0)
...
async def ParameterEmptyCheck(Parameter, ctx):
    ...
    try:
        ...
        logger.error(
            f"Uncaught Exception in Module : `ParameterEmptyCheck`, ErrorType : `{type(Exc)}` : {Exc}")
    else:
        return(0)
...
async def IntCheck(TypeObj):
    ...
    try:
        ...
        logger.error(
            f"Uncaught Exception in Module : `IntCheck`, ErrorType : `{type(Exc)}` : {Exc}")
    else:
        return(1)
...
class ToDoList(commands.Cog):
    ...
    async def MyLists(self, ctx):
        ...
        for _ in Lists:
            ...
            embed = discord.Embed(title="Your Lists", description="\n".join(
                DisplayableLists), color=0x0000ff)
    ...
    async def AuthMember(self, ctx, mode, user: discord.Member):
        ...
        try:
            if await IDCheck(ctx.author.id):
                raise IdError()
    ...
    async def ListAdd(self, ctx, *message: str):
        ...
        try:
            if await IDCheck(ctx.author.id):
                raise (IdError)
            if await ParameterEmptyCheck(message, ctx):
    ...
    async def List(self, ctx):
        try:
            ...
            with open(FileName, "r") as f:
                content = f.readlines()
                if await FileEmptyCheck(content, ctx):
                    raise(FileEmpty)
                for i, _ in enumerate(content, 1):
                    STP += f"{str(i).rjust(3)} : `{_}`"
                EmbedFileName = FileName.split("/")[2]
                EmbedFileName = EmbedFileName.replace(
                    f"-{ctx.author.id}.ToDo", "")
                embed = discord.Embed(
                    title=f"{EmbedFileName}", description=STP, color=0x0000ff)
                await ctx.send(embed=embed)
    ...
    async def RemoveAll(self, ctx):
        ...
        try:
            ...
            if Type(Exc) in [FileEmpty, IdError]:
                await ctx.send("You aren't permitted to use this command.")
            else:
                pass
        else:
    ...
    async def Remove(self, ctx, name=None):
        ...
        if name == None:
            await ctx.send("Name Parameter Not Set")
            return()
        ...
        if os.path.isdir(f"Lists/{ctx.author.id}"):
            try:
                ...
                if os.path.isfile(FileName):
                    await ctx.send(f"List Already Exists under name `{FileName}` ")
                    return()
